  ...
  def __call__(self, params):
    ...
    def _dataset_parser(value):
      ...
      with tf.name_scope('parser'):
        source_id = tf.random_uniform([1], maxval=10000, dtype=tf.int32)[0]
        image = tf.random_uniform(
        [640, 640, 3], minval=-0.5, maxval=0.5, dtype=tf.float32)
        boxes = tf.random_uniform([4], minval=0, maxval=10, dtype=tf.float32)
        classes = tf.random_uniform([1], minval=-1, maxval=1, dtype=tf.float32)
