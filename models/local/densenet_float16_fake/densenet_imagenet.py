...
#      lambda x: (tf.cast(tf.constant(np.zeros((224, 224, 3)).astype(np.float32), tf.float32), tf.float16),
#                 tf.random_uniform([1], maxval=_LABEL_CLASSES, dtype=tf.int32)[0]))
#
...
#    return images, labels
  ...
  def __call__(self, params):
    ...
    labels = tf.random_uniform(
        [batch_size], maxval=_LABEL_CLASSES, dtype=tf.int32) 
